...
class Softmax_Regression(object):
    ...
    def fit(self,train_PHI,train_t):
        ...
        while True:
            ...
            for cls_num in range(np.size(train_t, 1)):            #Updating weight matrix
                W_NEW[:,cls_num] = W[:,cls_num] - (np.linalg.inv(self.Hessian(train_PHI,Y,cls_num)) @ train_PHI.T @ (Y-train_t))[:,cls_num]
